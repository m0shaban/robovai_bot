...
def check_database():
    ...
    if not db_url:
        ...
        print(
            f"   Default: postgresql+asyncpg://postgres:postgres@localhost:5432/robovai"
        )
        return False
...
def start_backend():
    ...
    if not venv_python.exists():
        print(
            f"{Colors.RED}✗ Virtual environment not found at {venv_python}{Colors.END}"
        )
    ...
    try:
        ...
        if process.poll() is None:
            ...
            print(
                f"  • API Docs:   {Colors.BOLD}http://localhost:8000/docs{Colors.END}"
            )
            print(
                f"  • Health:     {Colors.BOLD}http://localhost:8000/health{Colors.END}"
            )
            return process
        else:
